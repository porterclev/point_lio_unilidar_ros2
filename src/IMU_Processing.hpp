...
void ImuProcess::IMU_init(const MeasureGroup &meas, int &N)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("IMU_Processing"), "IMU Initializing: %.1f %%", double(N) / MAX_INI_COUNT * 100);
  V3D cur_acc, cur_gyr;
...
void ImuProcess::Process(const MeasureGroup &meas, PointCloudXYZI::Ptr cur_pcl_un_)
{
  if (imu_en)
  {
    if (meas.imu.empty())
      return;
    RCLCPP_EXPORT(rclcpp::get_logger("IMU_Processing"), meas.lidar != nullptr);
    ...
    if (imu_need_init_)
    {
      ...
      {
        RCLCPP_INFO(rclcpp::get_logger("IMU_Processing"), "IMU Initializing: %.1f %%", 100.0);
